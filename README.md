...
A Next.js 14, Typescript, Clerk, Supabase, Phyziro(Electryt), OpenAi template ready to go.
...
- **Payment Processing**: Phyziro / Electryt


- edit image           -> fal-ai/flux/dev/image-to-image ✅
- image description    -> fal-ai/bagel/understand ✅
- image upscaler       -> fal-ai/clarity-upscaler ✅
- face swap            -> easel-ai/advanced-face-swap ✅

- image to video       -> fal-ai/pixverse/v4.5/image-to-video ✅
- avatar to video      -> fal-ai/ai-avatar/single-text ✅

- prompt to image      -> fal-ai/flux-pro/v1.1 ✅
- avatar to image      -> fal-ai/flux-lora/image-to-image ✅

- avatar trainning     -> fal-ai/flux-pro-trainer ✅
...
## Mission Control (Custom Tooling)

Mission Control lives at `/mission-control` and is built for internal workflow ops.

### Included tools
- **Prompt Studio** (`/mission-control/prompt-studio`)
  - Save reusable prompt templates by category.
- **Content Batch Planner** (`/mission-control/batch-planner`)
  - Track batch tasks with status progression (`todo` → `in-progress` → `done`).
- **Model Router** (`/mission-control/model-router`)
  - Pick a task type and get recommended model + tool.

### Architecture
- Pages: `app/(dashboard)/(routes)/mission-control/*`
- Reusable UI: `components/mission-control/*`
- Helpers: `lib/mission-control/storage.ts`

### Data persistence
Mission Control tools currently persist to browser `localStorage` for fast iteration.

### Extend with new tools
1. Create a new route under `app/(dashboard)/(routes)/mission-control/<tool>/page.tsx`.
2. Add shared UI in `components/mission-control/`.
3. Add state/storage helpers in `lib/mission-control/`.
4. Add the tool card on `/mission-control` dashboard.
